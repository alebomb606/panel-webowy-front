...
function DetailedEventRow({ event, interactions, refreshList, showEventDetails }: DetailedEventRowProps) {
  ...
  return (
    <EventContainer>
      <EventHeader>
        <HistoryName to="#" onClick={onClick} color={TrailerStates.toColor(event.type)}>
          {t(TrailerStates.toReadableName(event.type))}
        </HistoryName>
        <ResolveAlarmButton id={event.id} type={event.type} interactions={event.interactions} onClick={refreshList} />
        ...
        {event.location && event.location.name && <Location>{event.location.name}</Location>}
...
export default DetailedEventRow;
