...
export enum SensorTypes {
  ...
  trailerSignal = 'trailerSignal',
}

export namespace SensorTypes {
  ...
  export function fromApiPropertyName(name?: string): SensorTypes | undefined {
    switch (name) {
      ...
      case 'trailer_signal':
        return SensorTypes.trailerSignal;
    }
    return undefined;
  }
  export function toApiPropertyName(name: SensorTypes) {
    switch (name) {
      ...
      case SensorTypes.trailerSignal:
        return 'trailer_signal';
    }
  }
...
function sensor(state = initialState, action: Action = { type: '' }): SensorsState {
  switch (action.type) {
    ...
    case SensorActionTypes.fetchSensorSettingsSuccess: {
      const {
        ...
        sensor,
        id,
      }: { sensorId: SensorId; sensor: Sensor; settings: SensorSettings; id: TrailerId } = action.payload;
    ...
    case SensorActionTypes.fetchSensorEventsSuccess: {
      const {
        ...
        alarms,
        order,
      }: { id: TrailerId; alarms: { [key in SensorEventId]: SensorEvent }; order: SensorEventId[] } = action.payload;
      return {
        ...state,
    ...
    case SensorActionTypes.fetchSensorsSuccess: {
      const { id, sensors }: { id: TrailerId; sensors: SensorEntry; date: string } = action.payload;
      return {
        ...state,
        sensors: {
          ...state.sensors,
