...
import { SensorTypes, SensorEntry, SensorSettings, SensorId, Sensor } from './reducer';
...
export function fetchSensors(id: string) {
  return async (dispatch: ThunkDispatch<State, {}, Action>, getState: () => State) => {
    ...
    try {
      ...
      const sensors: SensorEntry = data.reduce((accumulator: any, sensor: any) => {
        try {
          const parsed = parseSensor(sensor);
          return {
            ...accumulator,
            [`${SensorTypes.fromApiPropertyName(sensor.kind)}`]: parsed,
          };
        } catch (error) {
          return accumulator;
        }
      ...
      dispatch({ type: SensorActionTypes.fetchSensorsFail, error: true, payload: { error } });
    }
  };
}
...
export function fetchSensorEvents(id: TrailerId, sensorId: SensorId) {
  return async (dispatch: ThunkDispatch<State, {}, Action>, getState: () => State) => {
    ...
    try {
      ...
      const payload = data.reduce(
        (accumulator: any, element: any) => ({
          ...
          alarms: {
            ...
            [element.id]: {
              id: element.id,
              type: SensorTypes.fromApiPropertyName(element.sensor_name),
              status: TrailerStates.from(element.kind),
      ...
      dispatch({ type: SensorActionTypes.fetchSensorEventsFail, error: true, payload: { error } });
    }
  };
}
...
function displayError(error: any) {
  try {
    let errors = JSON.parse(error.request.response).errors.map(({ title, detail }: any) => `${title}: ${detail}\n`);
    alert(errors);
  } catch (err) {
    alert('Unknown error');
  }
...
export function patchSensorSettings(sensorId: SensorId, settings: SensorSettings, id: TrailerId) {
  return async (dispatch: ThunkDispatch<State, {}, Action>, getState: () => State) => {
    const { auth } = getState();
    try {
      dispatch({ type: SensorActionTypes.patchSensorSettingsSent });
      const { data } = await api.patchSensorSettings({ sensorId, settings, auth });
      ...
      dispatch({
        ...
        payload: { sensorId, settings: updatedSettings, sensor: updatedSensor, id: id },
      });
    } catch (error) {
      displayError(error);
      dispatch({ type: SensorActionTypes.patchSensorSettingsFail, error: true, payload: { error } });
    }
  };
}
...
export function fetchSensorSettings(sensorId: SensorId, id: TrailerId) {
  return async (dispatch: ThunkDispatch<State, {}, Action>, getState: () => State) => {
    ...
    try {
      ...
      dispatch({ type: SensorActionTypes.fetchSensorSettingsFail, error: true, payload: { error } });
    }
  };
}
...
function parseSensor(data: any): Sensor {
  const type = SensorTypes.fromApiPropertyName(data.kind);
  if (!type) {
