...
import { Sensor as SensorData, SensorsState, SensorSettings, SensorTypes } from '../reducer';
...
export default function Chart({ sensors, id, sensor, settings, fetchSensors }: ChartProps) {
  const { t } = useTranslation('sensors');

  const snakecased = settings && camelToSnake(settings.sensorType);
  const isDataTransfer = SensorTypes.fromApiPropertyName(snakecased) === SensorTypes.dataTransfer;
  ...
  return (
    <ChartContainer>
      ...
      <DetailedChart entity={sensor} />
      <RefreshButton loading={sensors.loading || false} detailed={true} onClick={() => fetchSensors(id)} />
    </ChartContainer>
  );
}
