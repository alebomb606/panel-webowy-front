...
import { SensorTypes, Sensor } from '../reducer';
...
export default function Stats(props: DataProps) {
  ...
  const sensorUnit = SensorTypes.getSensorUnit(sensor.type);
  ...
  return (
    <SensorData>
      ...
      <DataContent>
        ...
        <DataValue>{sensor.averageValue !== null ? `${sensor.averageValue} ${sensorUnit}` : t`no_data`}</DataValue>
      </DataContent>
      <DataContent>
        <DataLabel>{t(`${type}.last`)}</DataLabel>
        <DataValue>{sensor.value !== null ? `${sensor.value} ${sensorUnit}` : t`no_data`}</DataValue>
      </DataContent>
    </SensorData>
  );
}
