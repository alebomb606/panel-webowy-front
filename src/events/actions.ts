...
import api, { EventsQuery } from '../api';
...
export function fetchEvents(id: TrailerId | undefined, filter: EventsQuery = defaultFilters()) {
  return async (dispatch: Dispatch, getState: () => State) => {
    ...
    try {
      ...
      const payload = data.reduce(mapEvent, { order: [], entities: {}, id: trailerId });
      dispatch({ type: TrailerEventsActionTypes.fetchEventsSuccess, payload });
    } catch (error) {
      dispatch({ type: TrailerEventsActionTypes.fetchEventsFail, error: true, payload: { error } });
    }
  };
}
...
export const mapInteraction = (interaction: any) => ({
  type: TrailerStates.from(interaction.kind),
...
export const mapEvent = ({ order, entities, id }: EventsAccumulator, event: any) => {
  ...
  if (event.route_log) {
    const { latitude, longitude, location_name: locationName, speed } = event.route_log;
    location = {
  ...
  return {
    ...
    entities: {
      ...
      [event.id]: {
        ...
        location,
        type: TrailerStates.from(event.kind),
