...
import { AlarmModalProps } from '../../trailers/container';
...
export function AlarmAlert({ closeModal, alarmModalProps }: AlarmAlertProps & OwnProps) {
  const { trailer, setTrailerState } = alarmModalProps;
  const { t } = useTranslation('alarm_modal');

  const possibilityOfLoudAlarm = trailer.state !== TrailerStates.alarm && trailer.state !== TrailerStates.emergency;
