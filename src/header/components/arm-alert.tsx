...
import { ArmModalProps } from '../../trailers/container';
...
export function Alert(props: AlertProps & OwnProps) {
  ...
  const { description, header = t`warning`, info = '', cancel = t`cancel`, confirm = t`save` } = labels;
  ...
  return (
    <Container>
      ...
      <Info>{info}</Info>
      <ButtonsWrapper>
        {cancel !== null && <CancelButton onClick={() => closeModal()}>{cancel}</CancelButton>}
        <ConfirmButton onClick={() => onConfirm(closeModal)}>{confirm}</ConfirmButton>
      </ButtonsWrapper>
    </Container>
  );
}
...
export function ArmAlert(props: ArmAlertProps & OwnProps) {
  ...
  const onConfirm = () => armSystemWithCameras(trailer, getArmedStatus, setTrailerState, closeModal);
  return (
    <Alert
      labels={{
        header: t`attention`,
