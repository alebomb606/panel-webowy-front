...
import { SensorId, SensorSettings } from './sensors/reducer';
...
export interface EventsQuery {
  from?: Date;
  to?: Date;
  kinds?: { [key in TrailerStates]: boolean };
  limit?: number;
}
...
export default {
  ...
  async post(url: string, params: URLSearchParams | {} | null, auth: AuthState, config: RequestOptions = {}) {
  ...
  async patch(url: string, params: Object, auth: AuthState, config: RequestOptions = {}) {
    const headers = this.getHeaders(auth);
  ...
  async fetchEvents({ id, filter, auth }: { id: TrailerId; filter: EventsQuery; auth: AuthState }) {
    const from =
    ...
    const filters = Object.entries(filter.kinds || {})
      .reduce<Array<String>>((keys, [key, val]) => {
        if (val && TrailerStates.isTrailerState(key)) {
          return [...keys, TrailerStates.toApiParam(key)];
        }
        return keys;
      }, [])
      .join(',');
    ...
    const params = this.getParams({
      ...
      'page[size]': typeof filter.limit === 'undefined' ? undefined : `${filter.limit}`,
    }).toString();

    return await this.get(URL.events(id, params), auth);
  },

  async fetchRoutes({ id, filter, auth }: { id: TrailerId; filter: RoutesQuery; auth: AuthState }) {
    const from =
      filter.from !== undefined
        ? moment(filter.from)
            .startOf('day')
  ...
  async fetchSensorEvents({ sensorId, auth }: { sensorId: SensorId; auth: AuthState }) {
  ...
  async fetchSensorSettings({ sensorId, auth }: { sensorId: SensorId; auth: AuthState }) {
    return await this.get(URL.sensorSettings(sensorId), auth);
  },

  async setTrailerState({ id, status, auth }: { id: TrailerId; status: TrailerStates; auth: AuthState }) {
    const params = {
      data: {
        type: 'trailer',
        id: id,
        attributes: {
          status: TrailerStates.toApiParam(status),
        },
      },
    };
  ...
  async fetchMedia({ id, query, auth }: { id: TrailerId; query: MediaListRequestQuery; auth: AuthState }) {
    const { from, to } = query;
    const params = this.getParams({
      'page[size]': '10000',
  ...
  async requestMedia({ id, query, auth }: { id: TrailerId; query: MediaRequestQuery; auth: AuthState }) {
    const params = {
      data: {
        id,
        type: 'trailer',
