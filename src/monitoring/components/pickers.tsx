...
export default function({ date, onChange, setCurrentTime }: Props) {
  ...
  const onPickerChange = (change: Date | Moment, pickerType?: 'day' | 'hour' | 'maxDate' | 'minDate') => {
    if (!change) return;
  ...
  return (
    <Pickers>
      <Label onClick={e => e.preventDefault()}>
        ...
        <Datepicker pickerType="day" date={date} onPickerChange={onPickerChange} validDate={validDate} />
      </Label>
      <Label onClick={e => e.preventDefault()}>
        <Caption>{t`hour`}</Caption>
        <Timepicker pickerType="hour" date={date} onPickerChange={onPickerChange} />
      </Label>
