...
const renderThumbs = ({ quantity, date, timeline, onThumbClick }: Props, half: number) =>
  new Array(quantity).fill(0).map((_, index) => {
    ...
    return <Thumb src={image && image.snapshotUrl} alarm={image && image.alarm} onClick={clickHandler} key={key} />;
  });
...
export default function(props: Props) {
  ...
  return (
    <Thumbnails>
      {[<PrevButton key="prev" onClick={prev} />, ...tiles, <NextButton key="next" onClick={next} />]}
    </Thumbnails>
  );
}
