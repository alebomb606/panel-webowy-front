...
const getTime = ({ monitoring }: State) => monitoring.ui.time || new Date().toISOString();
const getCamera = ({ monitoring }: State) => monitoring.ui.camera || MonitoringCameras.interior;
...
const getMediaForTrailer = (trailer: TrailerId | null, media: { [x: string]: TrailerMediaIds }) =>
...
export const getSnapshotForTime = createSelector(
  ...
  (trailer, images, media, time) => {
    ...
    const emptyImage = (type: MonitoringCameras): MonitoringCamera => {
      return MonitoringCameras.emptyImage(trailer || '', type, time);
    };
    ...
    const snapshot = {
      [MonitoringCameras.exterior]: map(find(MonitoringCameras.exterior), MonitoringCameras.exterior),
      [MonitoringCameras.interior]: map(find(MonitoringCameras.interior), MonitoringCameras.interior),
      [MonitoringCameras.leftBottom]: map(find(MonitoringCameras.leftBottom), MonitoringCameras.leftBottom),
      [MonitoringCameras.leftTop]: map(find(MonitoringCameras.leftTop), MonitoringCameras.leftTop),
      [MonitoringCameras.rightBottom]: map(find(MonitoringCameras.rightBottom), MonitoringCameras.rightBottom),
      [MonitoringCameras.rightTop]: map(find(MonitoringCameras.rightTop), MonitoringCameras.rightTop),
    };
    return snapshot;
  },
);
