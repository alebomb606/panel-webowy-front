...
import { getTrailersOrderFilteredByQuery, getTrailer } from '../trailers/selectors';
...
export const getRoutePoints = createSelector(
  [getTrailersOrderFilteredByQuery, getRoutes, getMinDate, getMaxDate, getTrailer],
  (filteredTrailerIds, routes, minDate, maxDate, trailer) =>
    Object.keys(routes)
      .filter(routeTrailer => (trailer ? routeTrailer === trailer : filteredTrailerIds.includes(routeTrailer)))
      .map(routeTrailer =>
