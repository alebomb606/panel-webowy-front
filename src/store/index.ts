...
import eventsReducer, { TrailerEventsState } from '../events/reducer';
import monitoringReducer, { MonitoringState } from '../monitoring/reducer';
import routesReducer, { RoutesState } from '../routes/reducer';
import sensorsReducer, { SensorsState } from '../sensors/reducer';
import trailersReducer, { TrailersState } from '../trailers/reducer';
...
const websocket: Middleware = _ => next => action => {
  if (action.payload && action.payload.meta && action.payload.meta.websocket && cableApp.network) {
...
export const store = createStore(reducers, composeWithDevTools(applyMiddleware(unauthorize, purge, websocket, thunk)));
